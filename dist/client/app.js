...
// ===== המשך חישוב הציון והתקדמות =====
function predictScore() {
    let score = 0;
    const activeUnits = (window.UNITS_DATA || []).filter(u => !u.excluded);
    const totalUnitsCount = activeUnits.length;

    activeUnits.forEach(u => {
        score += getUnitProgress(u.id);
    });

    const finalScore = totalUnitsCount > 0 ? Math.round(score / totalUnitsCount) : 0;
    
    // עדכון אלמנט הציון ב-UI אם קיים
    const scoreEl = document.getElementById('predicted-score');
    if (scoreEl) {
        scoreEl.textContent = finalScore + '%';
        scoreEl.className = 'score-badge ' + (finalScore > 80 ? 'high' : finalScore > 55 ? 'med' : 'low');
    }
    return finalScore;
}
...
function navigate(pageId, unitId = null) {
    // עדכון מצב האפליקציה
    STATE.lastTransitionPage = STATE.currentPage;
    STATE.currentPage = pageId;
    STATE.currentUnit = unitId;

    // ניקוי טיימרים אם עוזבים דף למידה
    if (pageId !== 'unit' && STATE.timerInterval) {
        clearInterval(STATE.timerInterval);
        STATE.timerActive = false;
    }

    // עדכון ה-URL (Hash) - זה יפעיל את ה-render באופן אוטומטי
    const hash = unitId ? `${pageId}/${unitId}` : pageId;
    window.location.hash = hash;
    
    // הנגשה: העברת פוקוס לראש הדף החדש
    window.scrollTo(0, 0);
    const mainContent = document.getElementById('main-content');
    if (mainContent) mainContent.focus();
    
    // הכרזה לקורא מסך
    announceToSR(`עברת לדף ${pageId}`);
}
...
function renderUnitList() {
    ...
    // שימוש ב-map במקום לולאות i/k ידניות מונע שגיאות "unused variable"
    container.innerHTML = (window.UNITS_DATA || []).map(unit => `
        <div class="unit-card" onclick="window.CivicsApp.navigate('unit', ${unit.id})">
            <span class="unit-icon">${unit.icon || '📚'}</span>
            <h3>${unit.title}</h3>
            <div class="unit-progress-mini">
                <div class="bar" style="width: ${getUnitProgress(unit.id)}%"></div>
            </div>
        </div>
    `).join('');
}
...
/**
 * Civics 2026 Core Engine - ASD Optimized
 * Features: Mnemonic Injection, Progress Tracking, Zero-Jank Rendering
 */
...
async function initApp() {
    try {
        ...
        // שיטוח המבנה מה-JSON לתוך מערך אחד נוח לעבודה
        AppState.allConcepts = data.units.flatMap(unit => 
            unit.concepts.map(c => ({ ...c, unitNumber: unit.unitNumber }))
        );
        
        AppState.filteredConcepts = [...AppState.allConcepts];
        ...
        document.getElementById('concepts-list').innerHTML = `<div class="error">שגיאה בטעינת הנתונים.</div>`;
    }
}
...
function updateProgressBar() {
    ...
    const percentage = Math.round((AppState.filteredConcepts.length / AppState.totalConcepts) * 100);
    ...
    if (text) {
        text.innerText = `מציג ${AppState.filteredConcepts.length} מתוך ${AppState.totalConcepts} מושגים (${percentage}%)`;
    }
}
...
function renderConcepts() {
    ...
    AppState.filteredConcepts.forEach(concept => {
        ...
        card.innerHTML = `
            <div class="mnemonic-box" role="note">
                💡 <strong>טיפ לזכירה:</strong> ${concept.memorySentence}
            </div>
            <h3 tabindex="-1">${concept.conceptName}</h3>
            <div class="concept-content">
                <p><strong>הגדרה:</strong> ${concept.definition || 'טוען הגדרה...'}</p>
            </div>
        `;
...
function setupEventListeners() {
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        if (!ticking) {
            window.requestAnimationFrame(() => {
                AppState.filteredConcepts = AppState.allConcepts.filter(c => 
                    c.conceptName.toLowerCase().includes(AppState.searchTerm)
                );
                renderConcepts();
                ticking = false;
            });
